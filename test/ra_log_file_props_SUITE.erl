...
last_written_with_crashing_segment_writer(Config) ->
    Dir = ?config(wal_dir, Config),
    TestCase = ?config(test_case, Config),
    run_proper(fun last_written_with_crashing_segment_writer_prop/2, [Dir, TestCase], 10).
    ...
    ?FORALL(
       ...
       ?FORALL(
          ...
          begin
              ...
              {Log, Last, Ts} =
                  lists:foldl(fun({wait, Wait}, Acc) ->
                                 ...
                                 (consume, {Acc0, Last0, Ts}) ->
                                      {Acc1, Last1} = consume_events(Acc0, Last0),
                                      {Acc1, Last1, Ts};
                                 (crash_segment_writer, {Acc0, Last0, Ts} = Acc) ->
              ...
              timer:sleep(time_diff_to(Ts, 6000)),
              %% Consumem all events
              {Log1, {LastIdx, _} = Last1} = consume_all_events(Log, Last),
              ...
              basic_reset(Log2),
              ?WHENFAIL(io:format("Last entry obtained: ~p; expected from events: ~p;"
                                  " last entry written: ~p~nEntries taken: ~p~n Actions: ~p~n",
                                  [Got, Last1, LastEntry, Written, All]),
                        (Got ==  Last1) and (Last1 == LastEntry)
                        and (Written == Entries))
          end)).
