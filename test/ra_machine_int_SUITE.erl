...
query_snapshot_size(Config) ->
    Mod = ?config(modname, Config),
    Self = self(),
    meck:new(Mod, [non_strict]),
    meck:expect(Mod, init, fun (_) -> the_state end),
    meck:expect(Mod, apply, fun
                                (_, {snapshot_size, _SnapshotSize} = LSS, State) ->
    ...
    meck:expect(Mod, handle_aux, fun
                                     (leader, cast, tick, AuxState, IntState) ->
                                         SnapshotSize = ra_aux:snapshot_size(IntState),
    ...
    try
        ...
        ct:pal(
          "Restart Ra server and check the latest snapshot size was "
          "recovered as well"),
        ra:stop_server(?SYS, ServerId),
        _ = ra:restart_server(?SYS, ServerId),
        _ = ra:members(ServerId),

        SnapshotSize = erlang:get(snapshot_size),
        erlang:erase(snapshot_size),
        ?assertMatch(_ when is_integer(SnapshotSize), SnapshotSize),
        ?assertMatch(
           {ok, #{log := #{snapshot_size := SnapshotSize}}, _},
