...
-record(?MODULE,
        {uid :: ra_uid(),
         ...
         latest_snapshot_size = undefined :: non_neg_integer() | undefined}).
...
begin_snapshot(#{index := Idx, term := Term} = Meta, MacMod, MacState, SnapKind,
    ...
    BgWorkFun = fun () ->
                        ...
                        SnapshotSize = case Mod:write(SnapDir, Meta, Ref, Sync) of
                                           ok ->
                                               %% The snapshot implementation
                                               %% didn't return the size of the
                                               %% data, let's estimate it.
                                               erlang:external_size(Meta);
                                           {ok, BytesWritten} ->
...
promote_checkpoint(PromotionIdx,
                   #?MODULE{module = Mod,
                            ...
                            %% promoted checkpoint?
                            latest_snapshot_size = SnapshotSize} = State0) ->
