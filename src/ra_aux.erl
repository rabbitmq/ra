...
machine_state(State) ->
    State#ra_server_state.machine_state.

-spec effective_machine_version(ra_aux:internal_state()) ->
    ra_machine:version().
effective_machine_version(#ra_server_state{cfg = Cfg}) ->
...
log_last_index_term(#ra_server_state{log = Log}) ->
...
log_fetch(Idx, #ra_server_state{log = Log0} = State)
  when is_integer(Idx) ->
    case ra_log:fetch(Idx, Log0) of
        {{Idx, Term, {'$usr', Meta, Cmd, _ReplyMode}}, Log} ->
            {{Term, Meta, Cmd}, State#ra_server_state{log = Log}};
        {_, Log} ->
...
log_stats(#ra_server_state{log = Log}) ->
    ra_log:overview(Log).
